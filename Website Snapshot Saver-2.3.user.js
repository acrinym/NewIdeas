...
// @version      2.3
// @description  Saves full website pages (HTML, JS, CSS, images) and form elements into a zip file, optimized for BMC Helix visibility and capture
// @author       You
// @match        *://*/*
...
(function() {
    ...
    const createFloatingEye = (targetDocument = document, retryCount = 0) => {
        ...
        style.textContent = `
            ...
            .popup label { display: block; margin: 5px 0; cursor: help; }
            .popup button { margin: 10px 5px 0 0; padding: 5px 10px; cursor: pointer; }
            .popup select { margin: 5px 0; width: 100%; }
            .icon-container {
            ...
            .progress { margin-top: 10px; font-size: 12px; color: #333; }
            .progress-bar { width: 100%; background: #eee; height: 5px; border-radius: 3px; overflow: hidden; }
    ...
    const setupEyeIcon = () => {
        ...
        checkIframes();
        setInterval(checkIframes, 2000); // Periodically check for new iframes
    ...
    function parseResourcesFromHtml(html, baseUrl, options, isDom = false) {
        ...
        const patterns = [
            ...
            { regex: /<object[^>]+data=["'](.*?)["']/gi, attr: 'data', type: 'object' },
