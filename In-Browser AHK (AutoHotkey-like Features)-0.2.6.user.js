...
// @version      0.2.6
// @description  Provides AutoHotkey-like hotstring, hotkey, workflow, and plugin functionality within the browser. With enhanced plugin settings, lifecycle hooks, list search, dynamic hotstring placeholders, cursor positioning, and more custom script examples.
...
// @grant        GM_deleteValue // Added back for plugin settings/data cleanup
...
(function() {
    ...
    const defaultHotstrings = [
        ...
        { id: `hs_${Date.now()+3}_${Math.random().toString(36).substring(2,7)}`, trigger: ';sig', replacement: 'Best regards,{ENTER}Justin{|}', caseSensitive: false, enabled: true },
        { id: `hs_${Date.now()+4}_${Math.random().toString(36).substring(2,7)}`, trigger: 'afaik', replacement: 'as far as I know', caseSensitive: false, enabled: true },
        { id: `hs_${Date.now()+5}_${Math.random().toString(36).substring(2,7)}`, trigger: 'idk', replacement: "I don't know", caseSensitive: false, enabled: true },
        { id: `hs_${Date.now()+6}_${Math.random().toString(36).substring(2,7)}`, trigger: 'ty', replacement: 'thank you', caseSensitive: false, enabled: true },
    ];
    ...
    const defaultHotkeys = [
        ...
        { id: `hk_${Date.now()+2}_${Math.random().toString(36).substring(2,7)}`, keys: 'Alt+Shift+H', action: 'highlightAllLinks', description: 'Highlight All Links on Page', enabled: true, customJS: null }, // COMMA ADDED HERE
        // --- NEW EXAMPLE HOTKEYS ---
        {
            id: `hk_${Date.now()+3}_${Math.random().toString(36).substring(2,7)}`,
            keys: 'Control+Alt+Y',
            action: CUSTOM_JS_ACTION_ID,
            description: 'Open YouTube',
            enabled: true,
            customJS: "window.open('https://www.youtube.com', '_blank');\nshowNotification('Opening YouTube...','info');"
        },
        {
            id: `hk_${Date.now()+4}_${Math.random().toString(36).substring(2,7)}`,
            keys: 'Control+Shift+I',
            action: CUSTOM_JS_ACTION_ID,
            description: 'Insert ISO Timestamp',
            enabled: true,
            customJS: "const now = new Date();\nconst timestamp = now.toISOString();\nconst activeEl = document.activeElement;\nif (activeEl && (activeEl.isContentEditable || activeEl.tagName === 'INPUT' || activeEl.tagName === 'TEXTAREA')) {\n    if (activeEl.isContentEditable) {\n        document.execCommand('insertText', false, timestamp);\n    } else {\n        const start = activeEl.selectionStart;\n        const end = activeEl.selectionEnd;\n        activeEl.value = activeEl.value.substring(0, start) + timestamp + activeEl.value.substring(end);\n        activeEl.selectionStart = activeEl.selectionEnd = start + timestamp.length;\n    }\n    showNotification('ISO Timestamp Inserted', 'success');\n} else {\n    GM_setClipboard(timestamp, 'text');\n    showNotification('ISO Timestamp copied (no active input).', 'info');\n}"
        },
        {
            id: `hk_${Date.now()+5}_${Math.random().toString(36).substring(2,7)}`,
            keys: 'Control+Alt+B', // Changed from D to avoid conflict with browser dev tools "Toggle Device Toolbar"
            action: CUSTOM_JS_ACTION_ID,
            description: 'Toggle Basic Page Dark/Light Style',
            enabled: true,
            customJS: "const key = 'inBrowserAhkBasicDarkStyle';\nif (document.body.dataset[key] === 'true') {\n    document.body.style.removeProperty('background-color');\n    document.body.style.removeProperty('color');\n    delete document.body.dataset[key];\n    showNotification('Basic Dark Style Off', 'info');\n} else {\n    document.body.style.setProperty('background-color', '#282c34', 'important');\n    document.body.style.setProperty('color', '#abb2bf', 'important');\n    document.body.dataset[key] = 'true';\n    showNotification('Basic Dark Style On', 'info');\n}"
        }
        // --- END NEW EXAMPLE HOTKEYS ---
    ];
    ...
    async function deletePluginSettings(pluginId) {
        try {
            // This function requires GM_deleteValue to be granted and available.
            // If not, it might throw an error or the catch block will be hit.
            ...
            // As a fallback, we could try setting it to null, but that doesn't remove the key.
            // GM_setValue(`${STORAGE_KEY_PLUGIN_SETTINGS_PREFIX}${pluginId}`, null);
        }
    }
    ...
    async function deletePluginStorageNamespace(pluginId) {
        ...
        await deletePluginSettings(pluginId); // This attempts to delete the main settings key.
    }
    ...
    // --- Hotstring Engine ---
    async function handleHotstringInput(event) {
        ...
        for (const hs of hotstrings) {
            ...
            if (currentBuffer.endsWith(trigger)) {
                let rawReplacementText = hs.replacement;
                const triggerLen = hs.trigger.length;
                let finalReplacementText = rawReplacementText;
                let cursorPositionOffset = -1;

                if (finalReplacementText.includes('{CLIPBOARD}')) {
                    if (navigator.clipboard && typeof navigator.clipboard.readText === 'function') {
                        try {
                            const clipboardText = await navigator.clipboard.readText();
                            finalReplacementText = finalReplacementText.replace(/{CLIPBOARD}/g, clipboardText || "");
                        } catch (err) {
                            console.warn("In-Browser AHK: Could not read clipboard using navigator.clipboard.readText(). Error:", err);
                            showNotification("Clipboard read error. Doc might need focus or permission denied. Check console.", "error", 3500);
                            finalReplacementText = finalReplacementText.replace(/{CLIPBOARD}/g, "[CLIPBOARD READ FAILED]");
                        }
                    } else {
                        console.warn("In-Browser AHK: navigator.clipboard.readText API not available in this context.");
                        showNotification("Clipboard API (readText) not available.", "error", 3000);
                        finalReplacementText = finalReplacementText.replace(/{CLIPBOARD}/g, "[CLIPBOARD API N/A]");
                    }
                }

                const now = new Date();
                finalReplacementText = finalReplacementText.replace(/{DATE}/g, now.toLocaleDateString(undefined, { year: 'numeric', month: '2-digit', day: '2-digit' }));
                finalReplacementText = finalReplacementText.replace(/{TIME}/g, now.toLocaleTimeString(undefined, { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false }));
                finalReplacementText = finalReplacementText.replace(/{DATE:([^}]+)}/g, (match, formatStr) => {
                    const year = now.getFullYear(); const month = (now.getMonth() + 1).toString().padStart(2, '0'); const day = now.getDate().toString().padStart(2, '0'); const shortYear = year.toString().substring(2);
                    switch (formatStr.toUpperCase()) {
                        case 'YYYY-MM-DD': return `${year}-${month}-${day}`; case 'MM/DD/YYYY': return `${month}/${day}/${year}`; case 'DD.MM.YYYY': return `${day}.${month}.${year}`; case 'MM-DD-YYYY': return `${month}-${day}-${year}`; case 'YYYYMMDD': return `${year}${month}${day}`; case 'YY-MM-DD': return `${shortYear}-${month}-${day}`;
                        default: return now.toLocaleDateString();
                    }
                });
                finalReplacementText = finalReplacementText.replace(/{TIME:([^}]+)}/g, (match, formatStr) => {
                    const hours = now.getHours().toString().padStart(2, '0'); const minutes = now.getMinutes().toString().padStart(2, '0'); const seconds = now.getSeconds().toString().padStart(2, '0'); let ampmHours = now.getHours(); const ampm = ampmHours >= 12 ? 'PM' : 'AM'; ampmHours = ampmHours % 12; ampmHours = ampmHours ? ampmHours : 12; const paddedAmpmHours = ampmHours.toString().padStart(2, '0');
                    switch (formatStr.toUpperCase()) {
                        case 'HH:MM:SS': return `${hours}:${minutes}:${seconds}`; case 'HH:MM': return `${hours}:${minutes}`; case 'HH.MM.SS': return `${hours}.${minutes}.${seconds}`; case 'HHMMSS': return `${hours}${minutes}${seconds}`; case 'H:M:S': return `${now.getHours()}:${now.getMinutes()}:${now.getSeconds()}`; case 'h:MM TT': return `${paddedAmpmHours}:${minutes} ${ampm}`;
                        default: return now.toLocaleTimeString();
                    }
                });

                finalReplacementText = finalReplacementText.replace(/{ENTER}/g, '\n');
                finalReplacementText = finalReplacementText.replace(/{TAB}/g, '\t');

                const cursorMarkerIndex = finalReplacementText.indexOf(CURSOR_POSITION_MARKER);
                if (cursorMarkerIndex !== -1) {
                    finalReplacementText = finalReplacementText.replace(CURSOR_POSITION_MARKER, "");
                    cursorPositionOffset = cursorMarkerIndex;
                }

                if (target.value !== undefined) {
                    const textBeforeCursor = target.value.substring(0, target.selectionStart - triggerLen);
                    const textAfterOriginalSelection = target.value.substring(target.selectionEnd);

                    target.value = textBeforeCursor + finalReplacementText + textAfterOriginalSelection;

                    const finalCursorPos = textBeforeCursor.length + (cursorPositionOffset !== -1 ? cursorPositionOffset : finalReplacementText.length);
                    target.selectionStart = target.selectionEnd = finalCursorPos;
                    ...
                    if (sel.rangeCount > 0) {
                        ...
                        if (target.contains(range.commonAncestorContainer)) {
                            ...
                            const textNode = document.createTextNode(finalReplacementText);
                            range.insertNode(textNode);

                            if (cursorPositionOffset !== -1) {
                                range.setStart(textNode, cursorPositionOffset);
                                range.setEnd(textNode, cursorPositionOffset);
                            } else {
                            ...
                            document.execCommand('insertText', false, finalReplacementText);
                        }
                    }
                }
                hotstringInputBuffer = '';
    ...
    function handleHotkey(event) {
        ...
        for (const hk of hotkeys) {
            ...
            .sort().join('+');
            const normalizedPressedKeys = pressedKeysStr.split('+').sort().join('+');
            ...
            if (normalizedHkKeys === normalizedPressedKeys) {
                if (isInputTarget && !hk.keys.match(/Control|Alt|Meta|Shift/i) && hk.keys.split('+').length === 1) {
                    ...
                    if (!['Escape', 'F1', 'F2', 'F3', 'F4', 'F5', 'F6', 'F7', 'F8', 'F9', 'F10', 'F11', 'F12', 'Tab', 'Enter', 'Backspace', 'Delete', 'ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight', 'Home', 'End', 'PageUp', 'PageDown', 'Insert', 'Pause', 'ScrollLock', 'PrintScreen', 'CapsLock', 'NumLock'].includes(singleKey)) {
                        if (singleKey.length === 1 && singleKey.match(/\S/)) {
                            continue;
                        }
                    }
                }
                ...
                if (hk.action === CUSTOM_JS_ACTION_ID && hk.customJS) {
                    ...
                    try {
                        if (availableActions[hk.action].isPluginAction) {
                            availableActions[hk.action].func(event);
                        } else {
    ...
    function createConfigPanel() {
        ...
        panel.innerHTML = `
            <div class="header">
                ...
                <button id="in-browser-ahk-close-btn" class="close-btn">×</button>
            </div>
            <div class="tabs">
            ...
            <div id="hotstrings-tab" class="tab-content active">
                ...
                <input type="text" id="hs-replacement" placeholder="Replacement (e.g., by the way, or text with {CLIPBOARD}, {DATE:YYYY-MM-DD}, {TIME:HH:MM}, {ENTER}, {TAB}, {|} for cursor)">
  ...
  "actions": [
    {
      ...
      "javascriptCode": "showNotification('Action from ' + pluginMeta.name + '! Setting: ' + settings.myText + ', Stored Value: ' + await helpers.getPluginStorage('someKey', 0)); await helpers.setPluginStorage('someKey', (await helpers.getPluginStorage('someKey', 0) || 0) + 1);"
            ...
            <div id="settings-tab" class="tab-content">
                ...
                <button id="setting-save-btn">Save Settings</button>
                 <div class="help-text">
                    ...
                    <br><br>Hotstring Placeholders:
                    <code>{CLIPBOARD}</code>, <code>{DATE}</code>, <code>{TIME}</code>,
                    <code>{DATE:YYYY-MM-DD}</code>, <code>{TIME:HH:MM:SS}</code>,
                    <code>{ENTER}</code> (newline), <code>{TAB}</code> (tab character),
                    <code>{|}</code> (cursor position).
                </div>
            </div>

            <button id="save-all-to-storage-btn" class="save-all-btn">Save All Configurations to Storage</button>
            <button id="export-config-btn" class="save-all-btn">Export Config (JSON)</button>
            <button id="export-bahk-btn" class="save-all-btn">Export Config (.bahk)</button>
            <input type="file" id="import-config-input" accept=".json" style="display:none;">
            <button id="import-config-btn" class="save-all-btn">Import Config</button>
            <input type="file" id="import-bahk-input" accept=".bahk" style="display:none;">
            <button id="import-bahk-btn" class="save-all-btn">Import .bahk</button>
            <button id="reset-config-btn" class="save-all-btn">Reset to Defaults</button>
        `;
        ...
        panel.querySelector('#export-config-btn').addEventListener('click', exportAllConfig);
        panel.querySelector('#export-bahk-btn').addEventListener('click', exportBahkConfig);
        panel.querySelector('#import-config-btn').addEventListener('click', () => panel.querySelector('#import-config-input').click());
        panel.querySelector('#import-config-input').addEventListener('change', (e) => {
            if (e.target.files.length > 0) {
                importConfigFile(e.target.files[0]);
                e.target.value = '';
            }
        });
        panel.querySelector('#import-bahk-btn').addEventListener('click', () => panel.querySelector('#import-bahk-input').click());
        panel.querySelector('#import-bahk-input').addEventListener('change', (e) => {
            if (e.target.files.length > 0) {
                importBahkFile(e.target.files[0]);
                e.target.value = '';
            }
        });
        panel.querySelector('#reset-config-btn').addEventListener('click', resetAllConfigs);

        refreshActionDropdown();
        renderConfigPanel();
        setupSearchFilters(panel);
    }
    ...
    function refreshActionDropdown() {
        ...
        if (!actionSelect.value && actionSelect.options.length > 0) {
    ...
    function saveHotstring() {
        ...
        if (hotstringSearchInput && hotstringSearchInput.value) {
    ...
    function saveHotkey() {
        ...
        if (hotkeySearchInput && hotkeySearchInput.value) {
    ...
    function editHotkey(id) {
        ...
        panel.querySelector('#hk-keys').focus();
        panel.querySelector('#hk-action').dispatchEvent(new Event('change'));
    }
                        ...
                        if (validatePluginData(pluginData)) {
                            ...
                            if (newPluginEntry.meta.enabled !== false) {
                                ...
                                if (isNewPlugin) {
                                    ...
                                    await executePluginHook(newPluginEntry, 'onEnable');
                                }
                            }
                            renderConfigPanel();
                            refreshActionDropdown();
    ...
    async function deletePlugin(pluginId) {
        ...
        if (pluginToDelete.meta.enabled !== false) {
        ...
        if (panel) {
            ...
            if (pluginSearchInput && pluginSearchInput.value) {
    ...
    async function togglePluginEnable(pluginId, isEnabled) {
        ...
        if (plugin) {
            const oldEnabledState = plugin.meta.enabled !== false;
            ...
            refreshActionDropdown();
            const panel = document.getElementById(CONFIG_PANEL_ID);
            if (panel) renderPluginList(panel);
    ...
    function openPluginSettingsModal(pluginId) {
        ...
        modal.querySelector('#save-plugin-settings-btn').addEventListener('click', async () => {
            const oldSettings = JSON.parse(JSON.stringify(plugin.currentSettings));
            const newSettingsFromForm = {};
            ...
            plugin.meta.settings.forEach(settingDef => {
                ...
                if (settingDef.type === 'checkbox') {
                    ...
                    if (isNaN(formValue)) formValue = settingDef.defaultValue;
                } else {
            ...
            await savePluginSettings(pluginId, plugin.currentSettings);
...
function escapeHtml(unsafe) {
    ...
    async function exportAllConfig() {
        const data = {
            ...
            plugins: plugins.map(p => ({
                ...
                actions: p.actions,
                rawJson: p.rawJson,
                currentSettings: p.currentSettings
            })),
        ...
        a.href = url;
        a.download = 'in-browser-ahk-config.json';
        document.body.appendChild(a);
        a.click();
        setTimeout(() => { document.body.removeChild(a); URL.revokeObjectURL(url); }, 0);
    }

    async function exportBahkConfig() {
        const data = {
            hotstrings,
            hotkeys,
            plugins: plugins.map(p => ({
        ...
        const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/bahk' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = 'in-browser-ahk-config.bahk';
        document.body.appendChild(a);
        a.click();
        setTimeout(() => { document.body.removeChild(a); URL.revokeObjectURL(url); }, 0);
    }
    ...
    async function importConfigFile(file) {
        try {
            ...
            if (Array.isArray(data.hotstrings)) hotstrings = data.hotstrings;
            if (Array.isArray(data.hotkeys)) hotkeys = data.hotkeys;
            if (data.settings) currentSettings = { ...defaultSettings, ...data.settings };
            if (Array.isArray(data.plugins)) {
                for (const plugin of plugins) {
                    ...
                    unregisterActionsForPlugin(plugin.meta.id);
                    await deletePluginSettings(plugin.meta.id);
                }
                plugins = [];
                for (const shell of data.plugins) {
                    let pluginData;
                    try { pluginData = shell.rawJson ? JSON.parse(shell.rawJson) : shell; } catch (e) { console.error('Invalid plugin data', e); continue; }
                    if (!validatePluginData(pluginData)) continue;
                    const entry = {
                        meta: { ...pluginData.pluginMeta, enabled: shell.meta ? shell.meta.enabled : pluginData.pluginMeta.enabled },
                        actions: pluginData.actions,
                        rawJson: shell.rawJson || JSON.stringify(pluginData),
                        currentSettings: shell.currentSettings || {}
                    };
                    plugins.push(entry);
                    if (entry.meta.enabled !== false) {
                        entry.actions.forEach(a => compileAndRegisterPluginAction(entry, a));
                        await executePluginHook(entry, 'onLoad');
                    }
                }
            }
            showNotification('Configuration imported. Remember to "Save All".', 'success');
            renderConfigPanel();
            refreshActionDropdown();
        } catch (err) {
            console.error('Import config error:', err);
            showNotification('Error importing config. Check console.', 'error');
        }
    }

    async function importBahkFile(file) {
        try {
            const text = await file.text();
            const data = JSON.parse(text);
            if (Array.isArray(data.hotstrings)) hotstrings = data.hotstrings;
            ...
            console.error('Import .bahk error:', err);
            showNotification('Error importing .bahk file. Check console.', 'error');
        }
    }

    async function resetAllConfigs() {
        if (!confirm('Are you sure you want to reset all configurations to defaults?')) return;
        for (const plugin of plugins) {
            if (plugin.meta.enabled !== false) {
                await executePluginHook(plugin, 'onDisable');
            }
        ...
        plugins = [];
        hotstrings = JSON.parse(JSON.stringify(defaultHotstrings));
        hotkeys = JSON.parse(JSON.stringify(defaultHotkeys));
        currentSettings = JSON.parse(JSON.stringify(defaultSettings));
        showNotification('All configurations reset. Remember to "Save All".', 'success');
        renderConfigPanel();
        refreshActionDropdown();
    }
    ...
    const GM_info = (typeof GM !== "undefined" && GM.info) ? GM.info : { script: { version: "0.2.6" } };
