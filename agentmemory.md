...
- **2026-03-12** — **Selective salvage from the conflicting remote branch has started.** Plugin categories and default-enable metadata now live in the current branch via `Cycloside/SDK/PluginCategory.cs`, `Cycloside/SDK/IPluginMetadata.cs`, and `Cycloside/Plugins/PluginMetadataResolver.cs`. First-run config now keys built-ins by stable plugin IDs such as `JezzballPlugin` instead of display names, with legacy display-name fallback during migration.
